...
import { Button, Card, Flex, Tooltip, Modal, message, Spin, Empty, Form } from "antd";
import {
  EditOutlined,
  DeleteOutlined,
  UserAddOutlined,
...
export default function TeamPage() {
  ...
  const [currentMember, setCurrentMember] = useState<TeamMember | undefined>(
    undefined
  );
  ...
  const handleAddSubmit = async (
    values: TeamMember & { password?: string }
  ) => {
    try {
      setIsLoading(true);
  ...
  return (
    <div className="bg-white p-6 rounded shadow-sm">
      <Flex justify="space-between" align="center">
        <h1 className="text-2xl font-bold mb-4.5">Team</h1>
        {canAddMembers && (
          <Button
            type="primary"
            icon={<UserAddOutlined />}
            onClick={handleAddMember}
            className="bg-blue-600 mb-4.5"
          >
            Add New Member
          </Button>
        )}
      ...
      {teamMembers.length === 0 ? (
        <Empty description="No team members found" className="my-12" />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {teamMembers.map((member) => (
            <Card
              key={member.id}
              hoverable
              className="w-full overflow-hidden transition-all duration-300 hover:shadow-sm"
              cover={
              <div className="relative flex justify-center items-center p-6 bg-gradient-to-r from-blue-500 to-purple-600">
              ...
              [
                ...
                <Tooltip key="edit" title="Edit Member">
                  <EditOutlined
                  onClick={() => handleEdit(member.id!.toString())}
                  />
                </Tooltip>
                ),
                ...
                <Tooltip key="delete" title="Delete Member">
                  <DeleteOutlined
                  onClick={() => showDeleteModal(member.id!.toString())}
